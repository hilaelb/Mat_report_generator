...
from rosbags.typesys import get_types_from_msg, get_typestore, Stores, register_types
...
# Import message types from the registered typestore
from rosbags.typesys.stores.ros2_foxy import (
    AdcData as AdcDataMsg,
    BaData as BaDataMsg,
    BmeData as BmeDataMsg,
    Vn200Data as Vn200DataMsg,
    InsStatus as InsStatusMsg,
    GnssVn200 as GnssVn200Msg,
    JetMotorData as JetMotorDataMsg,
    MotorGuidance as MotorGuidanceMsg,
    Joystick as JoystickMsg,
    ServoData as ServoDataMsg,
    TigerMode as TigerModeMsg,
    Parameter as ParameterMsg,
    Waypoint as WaypointMsg,
    PidLog as PidLogMsg,
    Status as StatusMsg,
    QgcMode as QgcModeMsg,
)
...
# Prompt the user to select the folder containing .rosbag files
folder_path = QFileDialog.getExistingDirectory(None, "Select the folder containing rosbag files")

if not folder_path:
    print("No folder selected. Exiting.")
    exit()

# List all the rosbag files in the selected directory
ROSBAG_NAMES = [f for f in os.listdir(folder_path) if f.endswith('.db3')]  # Assuming .db3 extension for rosbag2
...
def read_bag(file_name):
    ...
    with Reader(bag_file) as reader:
        ...
        adc = {'timestamp': [], 'ain0': [], 'ain1': [], 'ain2': [], 'ain3': [], 'ain4': [], 'ain5': [], 'ain6': [],
               'ain7': [], 'ain8': [], 'ain9': [], 'ain10': [], 'ain11': []}
        ...
        vn = {'timestamp': [], "time_startup": [], "yaw": [], "pitch": [], "roll": [], "poslla_x": [], "poslla_y": [],
              "poslla_z": [], "velned_x": [], "velned_y": [], "velned_z": [], "posu": [], "velu": []}

        joystick = {'timestamp': [], "ch1": [], "ch2": [], "ch3": [], "ch4": [], "ch5": [], "ch6": [], "ch7": [],
                    "ch8": []}

        jet = {'timestamp': [], "avg_motor_current": [], "rpm": [], "temp_fet": [], "temp_motor": [], "v_in": []}

        mogu = {'timestamp': [], "srv_1_angle": [], "srv_2_angle": [], "srv_3_angle": [], "srv_4_angle": [],
                "jet_rpm": []}

        param = {"id": [], "value": [], "type": [], "count": [], "index": []}

        servo = {'timestamp': [], "data_valid": [], "srv1_angle": [], "srv1_voltage": [], "srv1_temperature": [],
                 "srv2_angle": [], "srv2_voltage": [], "srv2_temperature": [], "srv3_angle": [], "srv3_voltage": [],
                 "srv3_temperature": [], "srv4_angle": [], "srv4_voltage": [], "srv4_temperature": []}
        ...
        waypoint = {"seq": [], "command": [], "alt_depth": [], "stay_at_pos_time": [], "uncertainty_radius": [],
                    "tool_speed": [], "latitude": [], "longitude": [], "altitude": []}
        ...
        data = {'adc': adc, 'ba30': ba30, 'bme_front': bme_front, 'bme_back': bme_back, 'vn': vn, 'joystick': joystick,
                'jet': jet, 'mogu': mogu, 'param': param, 'servo': servo, 'status': status, 'tiger': tiger, 'qgc': qgc,
                'waypoint': waypoint, 'insstatus': insstatus, 'gnssVn200': gnssVn200}
        output_mat_file = os.path.join(output_folder, os.path.basename(bag_file).replace('.db3', '.mat'))
        savemat(output_mat_file, data)
...
# Read and process each rosbag file
for rosbag_file in ROSBAG_NAMES:
    print(f"Processing: {rosbag_file}")
    read_bag(os.path.join(folder_path, rosbag_file))
